...
bool GLShader::init(const std::string &name,
    ...
    if (!geometry_str.empty())
        throw std::runtime_error("Geometry shaders are not supported on GLES!");
    mFragmentShader =
...
void GLShader::bind() {
    ...
    for (const auto &it : mBufferObjects) {
        const auto &buf = it.second;
        if (buf.attribID == -1) {
            glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buf.id);
        } else {
            glBindBuffer(GL_ARRAY_BUFFER, buf.id);
            glEnableVertexAttribArray(buf.attribID);
            glVertexAttribPointer(buf.attribID, (GLint) buf.dim, buf.glType, buf.compSize == 1 ? GL_TRUE : GL_FALSE, 0, 0);
        }
    }
}
...
void GLShader::downloadAttrib(const std::string &name, size_t size, int /* dim */,
    ...
    (void) name; (void) data;
    throw std::runtime_error("download_attrib(): unsupported on GLES!");
}
...
void GLFramebuffer::inittexture(const Vector2i &size) {
    mSize = size;
    glGenFramebuffers(1, &mFramebuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, mFramebuffer);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR); // for avoid nvidia bug https://www.khronos.org/opengl/wiki/Common_Mistakes#Render_To_Texture 
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, mSize.x(), mSize.y(), 0, GL_RGBA, GL_FLOAT, 0);
    glGenerateMipmap(GL_TEXTURE_2D);//mipmap can crash X-server using Chrome on Linux, GL_LINEAR without mipmap does not crash X 
...
void GLFramebuffer::bindtexture(const Vector2i &size, const int idx,const bool resizex) {
	glActiveTexture(GL_TEXTURE0+idx);
    glBindTexture(GL_TEXTURE_2D, texturebuffer);
	if((!(((mSize - size).norm() == 0)))&&(resizex))
	{
	mSize = size;
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, mSize.x(), mSize.y(), 0, GL_RGBA, GL_FLOAT, 0); // not glCopyTexSubImage2D or glTexSubImage2D
	glGenerateMipmap(GL_TEXTURE_2D);
	}
}

void GLFramebuffer::downloadTGA(const std::string &filename) {
	throw std::runtime_error("GLFramebuffer::download_tga(): Unimplemented for GLES!");
