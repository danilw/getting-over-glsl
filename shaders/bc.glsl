...
vec4 mi( vec2 fragCoord  ) {
	vec2 position = (( fragCoord.xy / iResolution.xy ) - vec2(0.5,0.5)) * 18.0;
	position.x = position.x*(iResolution.x / iResolution.y) - 0.5;
	vec4 color = vec4(0.50 , 0.3, 1.0* abs(fract(0.1 * iTime) - 0.5), 1.0);	//数字, 框架和圆点的颜色
	...
	//1. 分别作出四位数字, 每一位数字的变化周期都是其低一位的数字变化周期的10.0倍
    float utimex = texelFetch(iChannel0, ivec2(0), 0).x;
	number[0] = d_num(int(floor(10.0 * fract(10.0 * utimex))), position - vec2(1.5, 0.0), R);
	number[1] = d_num(int(floor(10.0 * fract(1.0 * utimex))), position - vec2(0., 0.0), R);
	number[2] = d_num(int(floor(10.0 * fract(0.1 * utimex))), position - vec2(-1.0, 0.0), R);
	number[3] = d_num(int(floor(10.0 * fract(0.01 * utimex))), position - vec2(-2.0, 0.0), R);
	//2. 计时器中间的两个点, 两点以1.0秒为1周期闪烁
	cycle = step(fract(iTime), fract(iTime + 0.5));
	//step(a, b)为阶梯比较函数如果(b<a), 返回0.0, 否则返回1.0, 这里有一半时间cycle的值为0.0, 其余为1.0
	number[4] = cycle * d_pot(vec2(-1., -0.25) + position, 0.02, R);
	number[5] = cycle * d_pot(vec2(-1., +0.25) + position, 0.02, R);
	...
	return clamp(color * light, 0.0, 1.0);
}
...
void main() {
    ...
    if(x>0.5)glFragColor=mi(gl_FragCoord.xy);
}
